...
Cilj aplikacije je omogućiti korisniku odabir i analizu portfelja kriptovaluta. Nakon
odabira portfelja korisniku se omogućuje detaljan pregled i analiza portfelja
kroz simulaciju budućih cijena portfelja, pregled simulacija i distribucija te
mogućnost optimiranja portfelja korištenjem PCA dekompozicije.\\
Na slici \ref{fig:model_aplikacije} možemo vidjeti kako aplikacija prvo zahtjeva
odabir portfelja kriptovaluta, zatim se parsiraju povijesni podatci kriptovaluta
koje su izabrane u portfelj. Nakon toga korisnik može odabrati ili Monte Carlo
simulaciju budućih cijena cijelog portfelja i pregled simulacija i distribucija
simulacija ili PCA dekompoziciju portfelja iz kojeg onda se generira grafički
prikaz optimalnog svojstvenog portfelja.\\
U nastavku slijedi detaljan opis svih funkcionalnosti aplikacije te
definicije klasa i funkcija koje su implementirane u aplikaciji. Logika
funkcija je opisana u tekstu uz definicije, a samu izvedbu funkcija je moguće
pronaći u kodu aplikacije koji je dostupan u prilogu rada.\\
...
U trenutnim podatcima nalaze se povijesne vrijednosti za 23 različite
kriptovalute:
\begin{itemize}
    \item Aave (AAVE)
    \item Bitcoin (BTC)
...
\item \textbf{Model kriptovaluta (\ref{sek:model_kriptovalute}):} Implementacija modela kriptovaluta.
    ...
    \item \textbf{Model portfelja (\ref{sek:model_portfelja}):} Definiranje i
        implementacija modela investicijskog portfelja.
...
je prvi korak u simulaciji i stvaranju modela portfelja.
Podatci se učitavaju iz CSV datoteka koje sadrže povijesne cijene
kriptovaluta. Povijesni podatci su preuzeti s \cite{kaggledataset}.
...
poravnanje vremenskih oznaka svih kriptovaluta tako da se uzima najraniji
početni trenutak u kojem su dostupni podatci za sve kriptovalute u portfelju.
U pravilu se kriptovalutama trguje
24/7, ali je problem bio što neke kriptovalute nisu postojale ili
nisu imale podatke za sve dane svih ostalih kriptovaluta.
U isječku \ref{lst:candle} prikan je kod strukture \emph{candle}.
...
i driftova. U isječku \ref{lst:kriptovaluta} prikazana je definicija
klase \emph{Cryptocurrency} koja predstavlja model kriptovalute.
...
U isječku \ref{lst:portfelj} prikazana je definicija klase \emph{Portfolio}
koja predstavlja model portfelja. Ovaj model omogućuje dodavanje i
uklanjanje kriptovaluta iz portfelja te izračunavanje potrebnih metrika.
...
samo standardnu biblioteku \texttt{C++}-a. Matrica je implementirana
kao vektor vektora, a vektor kao vektor.
U isječku \ref{lst:cpp_example} prikazane su definicije metoda koje
su implementirane u matematičkom okviru aplikacije.
...
kriptovaluta. Prije generiranja simulacija, aplikacija prvo prvo
parsira povijesne podatke kriptovaluta koje su izabrane u portfelj te
izračunava kovarijacijsku matricu.
Simulacije koriste model geometrijskog Brownovog gibanja
za generiranje budućih cijena te Cholesky dekompoziciju za
osiguranje uzoračke korelacije između kriptovaluta.
Na slici \ref{fig:monte_carlo} je prikazan model generiranja Monte Carlo
simulacija.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.7\textheight, keepaspectratio]{Figures/Monte_Carlo_explanation.png}
    \caption{Model generiranja Monte Carlo simulacija portfelja kriptovaluta}
    \label{fig:monte_carlo}
\end{figure}
\noindent Implementacija Monte Carlo simulacija prvo koristi metodu za izračunavanje
kovarijacijske matrice portfelja iz povijesnih podataka kriptovaluta.
Također se može postaviti početni datum simulacije (ako se izostavi, onda se
uzima se zadnji dan dostupan u podatcima), broj simulacija i
...
transformiraju u korelirane slučajne varijable s pomoću Cholesky dekompozicije.
...
\begin{figure}[H]
    ...
    \caption{Primjer statističke analize i distribucije simulacije portfelja
    kriptovaluta (500 simulacija, 60 dana)}
...
objašnjena u \ref{sek:svojstvena_dekompozicija}. Svaka glavna komponenta
se promatra zasebno i nema utjecaj na ostale glavne komponente.
Primjer rezultata PCA dekompozicije je dan na slici
\ref{fig:pca_example}. Na slici vidimo kako je svaka komponenta
označena različitom bojom. Također vidimo da je moguća situacija
u kojoj je neka težina u komponenti negativna, što znači da je
negativno korelirana s ostatkom portfelja te bi se trebala uzeti
kratka pozicija (eng. \textit{short position}) u toj kriptovaluti.
...
\begin{figure}[H]
    ...
    (ako se ne izabere početno vrijeme simulacije, uzima se zadnji dan iz
    podataka)}
...
\section{Funkcionalnost učitavanja i parsiranja podataka}
...
da rezerviramo memoriju za vektor \emph{candle} struktura
...
optimizacije su napravljene koristeći \emph{move semantics}
...
ako je inicijalni uzorak smislen i dovoljno velik.
Treba napomenuti kako bi Monte Carlo metoda modelirana s
...
stohastičkog procesa. Dodatan razlog je pretpostavka konstantne volatilnosti.
U realnosti, volatilnost kriptovaluta može značajno varirati i često je
grupirana u ciklusima.
Za duže periode se preporučuju
...
\begin{figure}[H]
    ...
    \caption{Primjer Monte Carlo simulacije portfelja jedne kriptovalute (XRP) (100
    simulacija, 60 dana, uzorački drift = 0.00154516, uzoračka volatilnost = 0.0727809)}
...
puno drugih faktora koji utječu na cijene kriptovaluta. Razlog zašto želimo
model koji na ovakav način predviđa buduće kretanje cijena je asimetričnost
povrata. Asimetričnost povrata se vidi u ovom primjeru:
\begin{align}
    \text{Početna vrijednost $= 100.0$}\\
    \text{Vrijednost nakon trenutka $t = 80.0 \left(-20.0 \%\right)$} \\
    \text{Vrijednost nakon trenutka $t+1 = 100.0 \left(+25\%\right)$}
\end{align}
Na slici \ref{fig:xrp_dist} možemo vidjeti izgled distribucije predviđenih
vrijednosti portfelja kriptovalute XRP za posljednji dan simulacije.
Primjećujemo da je distribucija asimetrična i da je većina vrijednosti u lijevom
repu distribucije. To znači da je model predvidio da će vrijednost portfelja
XRP pasti.
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{Figures/xrp_dist.png}
    \caption{Distribucija predviđenih vrijednosti kriptovalute XRP na zadnji dan simulacije
    (100 simulacija, 60 dana)}
    \label{fig:xrp_dist}
\end{figure}
Izgled kretanja
...
Treći primjer simulacije dan je na slici \ref{fig:monte_carlo_example3} te
distribucija vrijednosti portfelja na posljedni dan simulacija na slici
\ref{fig:BTC_dist}.
...
\begin{figure}[H]
    ...
    \caption{Primjer Monte Carlo simulacije portfelja jedne kriptovalute
    (Bitcoin)
    (100 simulacija, 60 dana, uzorački drift = 0.0017602, uzoračka volatilnost = 0.0426928)}
    \label{fig:monte_carlo_example3}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{Figures/bitcoin_dist.png}
    \caption{Distribucija predviđenih vrijednosti kriptovalute Bitcoin na zadnji dan simulacije
    (100 simulacija, 60 dana)}
    \label{fig:BTC_dist}
\end{figure}
...
jer je asimptotski najsporija operacija. Kao što je ranije spomenuto,
...
od detalja oko upravljanja prozorima i događajima.
...
matematičkih algoritama i optimizacijom pristupa \textit{Cache} memoriji.

...
razini C++ koda, kao što su \emph{SIMD} instrukcije i \emph{Cache}
optimizacije.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{Figures/gui2.png}
    \caption{Grafičko sučelje aplikacije za simulaciju portfelja kriptovaluta}
    \label{fig:portfolio_gui2}
\end{figure}
...
\chapter{Zaključak}
\label{pog:zakljucak}
U ovom radu prikazan je razvoj aplikacije za procjenu rizika u
investicijskim portfeljima kriptovaluta korištenjem Monte Carlo simulacija
...
i implementaciju modela. Implementacija je realizirana u C++ programskom
...
potrebno dodatno optimizirati algoritme, osobito za izračun svojstvenih
...
podatke i pretpostavke o stacionarnosti procesa, zbog čega rezultati
simulacija ne mogu u potpunosti predvidjeti buduće tržišne događaje.

Kao smjerovi daljnjeg razvoja predlaže se integracija naprednijih
statističkih i strojno-učenih modela, proširenje podrške za real-time
podatke te optimizacija postojećih algoritama korištenjem specijaliziranih
matematičkih biblioteka. Razvijeni programski okvir i biblioteka za rad s
